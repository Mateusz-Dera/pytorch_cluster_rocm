...
where `${CUDA}` should be replaced by either `cpu`, `cu118`, `cu126`, `cu128` depending on your PyTorch installation.
...
where `${CUDA}` should be replaced by either `cpu`, `cu118`, `cu124`, `cu126` depending on your PyTorch installation.
...
Ensure that at least PyTorch 1.4.0 (2.7.1 for ROCm) is installed and verify that `cuda/bin` and `cuda/include` are in your `$PATH` and `$CPATH` respectively (for CUDA), or that ROCm is properly installed (for ROCm), *e.g.*:
...
This fork of PyTorch Cluster has built-in support for AMD ROCm. When using PyTorch with ROCm, the library will automatically detect and use ROCm instead of CUDA. The CUDA kernels are automatically translated to HIP for ROCm compatibility.

**Requirements:**
- PyTorch built with ROCm support
- ROCm 6.3+ installed
- Tested GPU (gfx1100)
- Potentially working GPUs (gfx900, gfx906, gfx908, gfx90a, gfx942, gfx1030, gfx1100, gfx1101, gfx1102, gfx1200, gfx1201)

**Installation:**
```bash
# Install from source with ROCm support
pip install .
```
...
mkdir build
cd build
# Add -DWITH_CUDA=on for CUDA support or -DWITH_HIP=on for ROCm support if needed
cmake ..
make
make install
```
